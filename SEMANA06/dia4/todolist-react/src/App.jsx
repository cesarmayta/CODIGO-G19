...
class App extends React.Component{
  ...
  mostrar(cod,index){
    axios.get('http://localhost:8000/tarea/'+cod)
    .then(res=>{
      this.setState({
        pos:index,
  ...
  guardar(e){
    ...
    if(cod>0){
      //actualizar
      axios.put('http://localhost:8000/tarea/'+cod,dataTarea)
      .then(res=>{
        let indx = this.state.pos;
        this.state.tareas[indx] = res.data;
      ...
      //insertar
        axios.post('http://localhost:8000/tarea/',dataTarea)
      .then(res=>{
        console.log(res.data)
        this.state.tareas.push(res.data)
        var temp = this.state.tareas
        this.setState({
          descripcion:'',
          tareas:temp,
  ...
  componentDidMount(){
    ...
    axios.get('http://localhost:8000/tarea')
    .then(res=>{
      console.log(res.data);
      this.setState({
        tareas : res.data
